...
public class RiffWave extends Riff {
	...
	public void set16bitSamples(float[] samples) {
		...
		for(int i = 0; i < samples.length; i++) {
			...
			short value = (short) Math.min(Math.max(sample, Short.MIN_VALUE), Short.MAX_VALUE);
	...
	public void set24bitSamples(float[] samples) {
		if(getBitsPerSample() != 24) {
			throw new IllegalStateException(
					"cannot get 24bit samples for " + getBitsPerSample() + "bit sample data");
		}
		byte[] rawSamples = new byte[samples.length * 3];
		for(int i = 0; i < samples.length; i++) {
			int value = (int) (samples[i] * 8388608.0f);
			if(value > 8388607) {
				value = 8388607;
	...
	public float[][] getFloatSamples() {
		...
		if(bits == 8) {
			...
			for(int ch = 0; ch < nch; ch++) {
				for(int i = 0; i < samples[ch].length; i++) {
					int off = i * nch + ch;
					samples[ch][i] = Endianess.gets24bitLE(rawSamples, 3 * off) / 8388608.0f;
				}
			}
			return samples;
		} else if(bits == 32) {
